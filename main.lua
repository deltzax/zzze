-- LocalScript Singularity_Hub (version corrigée)
-- Remplace entièrement ton LocalScript par celui-ci
...
-- Connexions pour surveiller les changements de Rewound (maintient plusieurs handlers)
...
local function CleanupEverything()
    ...
    for plr, conns in pairs(RewoundChangedConnections) do
        if conns then
            ...
            if conns.Watcher then pcall(function() conns.Watcher:Disconnect() end) end
        end
    end
...
-- === SYSTÈME DE VOL INFINITE YIELD (sans A/E) ===
local iyflyspeed = 1
local flyKeyDown, flyKeyUp

-- Fonction utilitaire pour obtenir la racine du personnage
local function getRoot(char)
    return char:FindFirstChild("HumanoidRootPart") or char:WaitForChild("HumanoidRootPart", 2)
end

-- Système de vol Infinite Yield sans montée/descente
function sFLY()
    ...
    local CONTROL = {F = 0, B = 0, L = 0, R = 0} -- Supprimé Q et E
    local lCONTROL = {F = 0, B = 0, L = 0, R = 0} -- Supprimé Q et E
    local SPEED = 0
    ...
    local function FLY()
        ...
        task.spawn(function()
            repeat task.wait()
                ...
                -- Calcul de la vitesse uniquement basé sur les touches horizontales
                ...
                -- Calcul du mouvement uniquement horizontal
                if (CONTROL.L + CONTROL.R) ~= 0 or (CONTROL.F + CONTROL.B) ~= 0 then
                    -- Mouvement horizontal seulement (pas de composante Y)
                    local moveDirection = ((camera.CFrame.LookVector * (CONTROL.F + CONTROL.B)) + 
                                         (camera.CFrame.RightVector * (CONTROL.L + CONTROL.R))).Unit
                    BV.Velocity = moveDirection * SPEED
                    lCONTROL = {F = CONTROL.F, B = CONTROL.B, L = CONTROL.L, R = CONTROL.R}
                elseif (CONTROL.L + CONTROL.R) == 0 and (CONTROL.F + CONTROL.B) == 0 and SPEED ~= 0 then
                    -- Continuer dans la dernière direction
                    local moveDirection = ((camera.CFrame.LookVector * (lCONTROL.F + lCONTROL.B)) + 
                                         (camera.CFrame.RightVector * (lCONTROL.L + lCONTROL.R))).Unit
                    BV.Velocity = moveDirection * SPEED
                else
                    BV.Velocity = Vector3.new(0, 0, 0)
                end
                
                -- Orientation suivant la caméra
    ...
    -- Gestion des touches (sans Q/E)
    flyKeyDown = AddConnection(UserInputService.InputBegan:Connect(function(input, processed)
        ...
        elseif input.KeyCode == Enum.KeyCode.LeftShift then
            -- Sprint : double la vitesse
            CONTROL.F = CONTROL.F * 2
            CONTROL.B = CONTROL.B * 2
            CONTROL.L = CONTROL.L * 2
            CONTROL.R = CONTROL.R * 2
        end
    end), "Fly")

    flyKeyUp = AddConnection(UserInputService.InputEnded:Connect(function(input, processed)
        if processed then return end
        ...
        elseif input.KeyCode == Enum.KeyCode.LeftShift then
            ...
            if CONTROL.F ~= 0 then CONTROL.F = iyflyspeed end
            if CONTROL.B ~= 0 then CONTROL.B = -iyflyspeed end
            if CONTROL.L ~= 0 then CONTROL.L = -iyflyspeed end
            if CONTROL.R ~= 0 then CONTROL.R = iyflyspeed end
        end
    end), "Fly")
...
-- === FONCTIONS SÉPARÉES POUR FULLBRIGHT ET NOFOG ===
local FullBrightEnabled = false
local NoFogEnabled = false
local originalBrightness = Lighting.Brightness
local originalFogEnd = Lighting.FogEnd
...
local function toggleFullBright()
    ...
    if FullBrightEnabled then
        ...
        -- Boucle pour maintenir FullBright activé (SANS toucher au brouillard)
        AddConnection(RunService.Heartbeat:Connect(function()
            ...
            -- Détruire seulement les atmosphères (pas le brouillard)
...
local function toggleNoFog()
    ...
    if NoFogEnabled then
        ...
        -- Boucle pour maintenir NoFog
        AddConnection(RunService.Heartbeat:Connect(function()
            if not NoFogEnabled then return end
            Lighting.FogEnd = 100000
            
            -- Supprimer les atmosphères (pour NoFog c'est cohérent)
            for _, v in pairs(Lighting:GetDescendants()) do
...
local function monitorRewoundChanges(plr)
    if RewoundChangedConnections[plr] then
        local conns = RewoundChangedConnections[plr]
        if conns.Added then pcall(function() conns.Added:Disconnect() end) end
        if conns.Removed then pcall(function() conns.Removed:Disconnect() end) end
        if conns.Watcher then pcall(function() conns.Watcher:Disconnect() end) end
    ...
    local backpack = plr:FindFirstChild("Backpack")
    if backpack then
        local addedConn = AddConnection(backpack.ChildAdded:Connect(function(child)
            if child.Name == "Rewound Time" then
                RewoundCache[plr] = nil
                if RewoundFrame and RewoundFrame.Visible then
                    updateRewoundList()
                end
            end
        end), "Rewound")

        local removedConn = AddConnection(backpack.ChildRemoved:Connect(function(child)
            if child.Name == "Rewound Time" then
                RewoundCache[plr] = nil
                if RewoundFrame and RewoundFrame.Visible then
                    updateRewoundList()
        ...
        RewoundChangedConnections[plr] = {Added = addedConn, Removed = removedConn}
    else
        local watcher = AddConnection(plr.ChildAdded:Connect(function(child)
            if child.Name == "Backpack" then
                monitorRewoundChanges(plr)
            end
        end), "Rewound")
        RewoundChangedConnections[plr] = {Watcher = watcher}
    end
end
...
local function removeESP(plr)
    ...
    if CharacterAddedConnections[plr] then pcall(function() CharacterAddedConnections[plr]:Disconnect() end) CharacterAddedConnections[plr] = nil end
        ...
        if plr~=player then
            if ESPenabled then
                ...
                if not CharacterAddedConnections[plr] then
                    CharacterAddedConnections[plr] = AddConnection(plr.CharacterAdded:Connect(function()
                        if ESPenabled then
                            task.wait(1)
                            createESP(plr)
                        end
...
NoclipBtn.MouseButton1Click:Connect(function()
    if not Noclipping then
        Noclipping=AddConnection(RunService.Stepped:Connect(function()
        ...
        if Noclipping then pcall(function() Noclipping:Disconnect() end) Noclipping=nil end
        Clip=true
...
TravellerBtn.MouseButton1Click:Connect(function()
    local npc = workspace:FindFirstChild("NPCS") and workspace.NPCS:FindFirstChild("Traveling Pawner")
...
local function updateSingularityList()
    ...
    for _, plr in ipairs(players) do
        ...
        if singularity and singularity ~= "No Value" then
            ...
            hasSingularityCount = hasSingularityCount + 1
        else
...
local function updateRewoundList()
    ...
    for _, plr in ipairs(players) do
        ...
        if count > 0 then
            hasRewoundCount = hasRewoundCount + 1
        end
